...
// fetchAURInfo fetches AUR info for a package (including dependencies)
...
// resolveDependencies recursively resolves dependencies for a list of packages
func resolveDependencies(pkgs []string, seen map[string]bool) ([]string, error) {
	...
	for _, pkg := range pkgs {
		...
		if deps, ok := info["Depends"].([]interface{}); ok {
			...
			depOrder, err := resolveDependencies(depNames, seen)
			if err != nil {
				return nil, err
			}
